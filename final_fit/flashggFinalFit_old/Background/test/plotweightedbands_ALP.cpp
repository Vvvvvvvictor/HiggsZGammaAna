...
int main(int argc, char *argv[]) {
  ...
  RooRealVar *mass = win->var("CMS_hza_mass");//bing
  mass->SetTitle("#m_{ll#gamma#gamma}");
  mass->setUnit("GeV");
  ...
  for (int i=0; i<chan->numTypes(); ++i) {
    ...
    std::cout << "[INFO] number of events  sbevents  " << sbevents << " bevents " << bevents << " sevents " << sevents << std::endl;
  ...
  //bing
  TH1D *hsig = new TH1D("hsig","",3200,lowedge,180.);
  TH1D *hwsig = new TH1D("hwsig","",3200,lowedge,180.);
  
  TH1D *hbkg = new TH1D("hbkg","",3200,lowedge,180.);
  TH1D *hwbkg = new TH1D("hwbkg","",3200,lowedge,180.);
  ...
  //bing
  TH1D *hsigbkg = new TH1D("hsigbkg","",3200,lowedge,180.);
  TH1D *hwsigbkg = new TH1D("hwsigbkg","",3200,lowedge,180.);   
  ...
  for (int i=0; i<chan->numTypes(); ++i) {
    ...
    TH1D *hsigplot_cat = new TH1D(TString::Format("hsigplot_cat%s",catnames[i].Data()),"",3200,lowedge,180.);//bing
 ...
 std::cout << "[[DEBUG]]" << "quantplusone"<< quantplusone << "quantplustwo" <<quantplustwo<< std::endl;  
...
//     pad2->SetBorderMode(0);
    ...
    TLegend *leg2 = new TLegend(0.58,0.5+offset,0.860,0.85);  
    ...
    //if (quoteMu_){
    //lat2->DrawLatex(0.159,0.8,Form("#hat{m}_{H}=%.1f GeV, #hat{#mu}=%.3f",MH->getVal(),r->getVal()));
    //} else {
    ...
    mytext->DrawLatex(0.6,0.93,Form(" %.0f fb^{-1} (13#scale[0.75]{ }TeV)",intLumi_));//bing        
    //mytext->DrawLatex(0.129+0.03,0.85,"H#rightarrow#gamma#gamma");
    mytext->DrawLatex(0.17,0.82,("m_{a} = "+to_string(int(mavalue_))+" GeV").c_str());//bing
    ...
    lat4->DrawLatex(0.535,0.87,TString::Format("B component subtracted"));
    
    ccat->cd();
    
    ccat->SaveAs(prefix+catnames[i]+".pdf");
    ccat->SaveAs(prefix+catnames[i]+".png");
    ccat->SaveAs(prefix+catnames[i]+".eps");
