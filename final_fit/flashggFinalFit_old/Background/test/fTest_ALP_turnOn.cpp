...
RooAbsPdf* getPdf(PdfModelBuilder &pdfsModel, string type, int order, int cat, const char* ext=""){
  ...
  else if (type=="Exponential") return pdfsModel.getExponentialStepxGau(Form("%s_exp%d",ext,order),order,cat);//bing
  ...
  else if (type=="PowerLaw") return pdfsModel.getPowerLawStepxGau(Form("%s_pow%d",ext,order),order,cat);//bing
  ...
  else if (type=="Laurent") return pdfsModel.getLaurentStepxGau(Form("%s_lau%d",ext,order),order,cat);//bing
...
vector<string> flashggCats_;
 bool isData_ =0;
  ...
  desc.add_options()
    ...
    ("ncats,n", po::value<int>(&ncats)->default_value(5),                                       "Number of categories")
    ("cat,c", po::value<int>(&catGlobal)->default_value(5),                                       "Order of categories")
  ...
  std::string ext = channelName.c_str();//FIXED
    ...
    std::cout << "[INFO] opened data entries: sumEntries: "<< dataFull->sumEntries()<<std::endl;
        ...
        //while (prob<0.05 && order < 4){ //FIXME
				RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,catGlobal,Form("ftest_pdf_%d_%s",cat,ext.c_str()));
				if (!bkgPdf){
          ...
          RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,catGlobal,Form("env_pdf_%s",ext.c_str()));//bing
					if (!bkgPdf ){
						// assume this order is not allowed
