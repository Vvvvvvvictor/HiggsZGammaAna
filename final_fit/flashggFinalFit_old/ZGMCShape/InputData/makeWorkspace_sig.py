...
for mass_H in [125 - shift, 125, 125 + shift]:
    ...
    data_mass_cats = {}
    for c in range(nCat):
        data_mass_cats[c] = RooDataSet("ggh_{0}_13TeV_cat0".format(mass_H),"ggh_{0}_13TeV_cat0".format(mass_H), ArgSet, "CMS_hzg_weight")
    ...
    for c in range(nCat):
        data_mass_cats[c].plotOn(massDist)
    massDist.Draw()

    for c in range(nCat):
        getattr(w,'import')(data_mass_cats[c])

    w.writeToFile(args.out+"/HZGamma_data_sig_Hm{0}_workspace.root".format(mass_H))
    del w
