...
print(" ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ HZG T2W RUN II ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ")

def get_options():
  parser = OptionParser()

  parser.add_option('--mass_ALP', dest='mass_ALP', default=1, type='int', help="ALP mass") # PZ
  parser.add_option('--year', dest='year', default='2017', help="Comma separated list of years")
  parser.add_option("--channel", dest='channel', default='', help="ele or mu") # PZ
  parser.add_option('--cat', dest='cat', default='', help="Comma separated list of categories")
...
inputDatacard = f"./output_Datacard/pruned_datacard_{opt.year}{extStr}_{opt.channel}.txt"
outputWSFile = f"./output_datacard_rootfile_{opt.channel}/Datacard_{opt.year}{extStr}_{opt.channel}_{opt.mode}.root"
print(f" --> Input: {inputDatacard} --> Output: {outputWSFile}")
...
fsub = open("./t2w_jobs_%s/t2w_%s_%s%s_%s.sh"%(opt.channel,opt.mode,opt.year,extStr,opt.channel),"w")
fsub.write("#!/bin/bash\n\n")
fsub.write("cd %s\n\n"%os.environ['PWD'])
...
fsub.write("text2workspace.py %s -o %s %s %s"%(inputDatacard,outputWSFile,opt.common_opts,models[opt.mode]))
fsub.close()

# Change permission for file
os.system("chmod 775 ./t2w_jobs_%s/t2w_%s_%s%s_%s.sh"%(opt.channel,opt.mode,opt.year,extStr,opt.channel))
...
if opt.batch == "condor": 
  if os.environ['PWD'].startswith("/eos"):
    ...
    subcmd = "condor_submit ./t2w_jobs_%s/t2w_%s_%s_%s.sub"%(opt.channel,opt.mode,opt.year,opt.channel)
elif opt.batch == 'local': subcmd = "bash ./t2w_jobs_%s/t2w_%s_%s%s_%s.sh"%(opt.channel,opt.mode,opt.year,extStr,opt.channel)
