...
parser.add_option("-m","--mass",type="int",default=125,help="Mass at which to calculate the systematic variations (default: %default)")
...
inWS = WSTFileWrapper(options.infilename,"wsig_13TeV")#FIXED
...
##############################################################################
result ={}
mass = inWS.var("CMS_hza_mass")#FIXED
  ...
  for c in options.cats:
    for p in options.procs:
      ...
      if p in bkgProcs:
        ...
        outFile.write('%8.7f '%(sumEntries/norm))#bing
  outFile.write('\n')
  outFile.write('\n')
###############################################################################
...
if ((options.justThisSyst== "batch_split") or options.justThisSyst==""):
  fff = r.TFile.Open(options.infilename)#bing
  www = fff.Get("wsig_13TeV")#bing
  dataNOMINAL =  www.data("sig_%s_mass_m%d_%s"%("ggh",options.mass,"cat0")) #bing
  NORM = www.function("hggpdfsmrel_13TeV_%s_%s_norm"%("ggh", "cat0"))#bing

  print "hggpdfsmrel_13TeV_%s_%s_norm"%("ggh", "cat0")
  print "sig_%s_mass_m%d_%s"%("ggh",options.mass,"cat0")
  norm = NORM.getVal()
  sumEntries = dataNOMINAL.sumEntries()
  print "[[ INFO ]] norm: " + str(norm) + " ; sumEntries: " + str(sumEntries)
  printPreamble()
