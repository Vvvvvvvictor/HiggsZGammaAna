...
def sumGaussians(mass, param, scale):
    ...
    sigma_float = RooRealVar("sigma_float","sigma_float",0.0,-0.1,.1)
...
def draw_dataModel(mass,a_mass,hist,model,name,path_out,year):
    ...
    latex.DrawLatex(0.8, 0.9, "#chi^2 /DOF={0}".format(round(chisquare,3)))

    if not os.path.exists(path_out+"M"+str(a_mass)):
        os.makedirs(path_out+"M"+str(a_mass))
    canv.SaveAs(path_out+"M"+str(a_mass)+"/DataModel_ScaleSmear_m"+str(a_mass)+"_"+year+"_"+name+".png")


def draw_hists(mass,hists,a_mass,year,path_out,sysList):
    canv = TCanvas("c1","c1",800,1000)
    frame = mass.frame()
    legend = TLegend(0.55,0.55,0.89,0.89)
    h_legend = {}
    for h in sysList:
        #h_hist.plotOn(frame, RooFit.LineColor(colors[hists.keys().index(h)]))
        hists[h].plotOn(frame, RooFit.MarkerColor(colors[hists.keys().index(h)]))

        h_legend[h] = TH1F("h_legend_"+h,"h_legend_"+h,1,0,1)
        h_legend[h].SetLineColor(colors[hists.keys().index(h)])
        legend.AddEntry(h_legend[h],h,'L')
    ...
    if not os.path.exists(path_out+"M"+str(a_mass)):
        os.makedirs(path_out+"M"+str(a_mass))
    canv.SaveAs(path_out+"M"+str(a_mass)+"/Data_ScaleSmear_m"+str(a_mass)+"_"+year+".png")
    canv.Clear()
...
def main():
    ...
    path_out = '/afs/cern.ch/work/z/zewang/private/flashggfit/CMSSW_10_2_13/src/flashggFinalFit/Signal/ALP_sys_UL/shapeSys/fit_run2_UL_ScaleSmear_ele/ScaleSmear_plot_check/'
    ...
    for a_mass in a_masses:
        for year in years:
            ...
            for sys in sysList:
                ...
                draw_dataModel(mass_,a_mass,data[sys],model[sys],sys,path_out,year)
            
            #draw_hists(mass_,data,a_mass,year,path_out,sysList)
