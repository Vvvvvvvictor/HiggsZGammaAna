...
if opt.cats in ['all','wall']:
  ...
  for cat in opt.cats.split(","):
    for year in opt.years.split(","):
      f = f"{swd__}/outdir_{opt.ext}/signalFit/output/CMS-HZG_sigfit_{year}_{cat}_{opt.channel}_Hm125.root"
      inputFiles[f"{year}_{cat}"] = f
      if citr == 0:
        w = ROOT.TFile(f).Get("wsig_13TeV")
        xvar = w.var(opt.xvar.split(":")[0])
        xvar.setPlotLabel(opt.xvar.split(":")[1])
...
# Make plot
if not os.path.isdir("%s/outdir_%s/Plots"%(swd__,opt.ext)): os.system("mkdir %s/outdir_%s/Plots"%(swd__,opt.channel))
plotSignalModel(hists,opt,_outdir="%s/outdir_%s/Plots"%(swd__,opt.ext), _Amass=opt.mass_ALP,_year=opt.years,_channel=opt.channel)
