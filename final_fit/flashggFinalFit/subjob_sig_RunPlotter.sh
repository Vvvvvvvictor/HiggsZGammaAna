...
for ((iChannel=0; iChannel<$nChannel; iChannel++))
  ...
  for ((iALPmass=0; iALPmass<$nALPmass; iALPmass++))
    ...
    for ((iYear=0; iYear<$nYear; iYear++))
      do
        hep_sub $path_run/runjob_sig_RunPlotter.sh -g cms -mem 2000 -wt mid -o $path_sig_log/${ChannelList[$iChannel]}/${ALPmassList[$iALPmass]}_RunPlotter_job_${YearsList[$iYear]}_${ChannelList[$iChannel]}.log -e $path_sig_log/${ChannelList[$iChannel]}/${ALPmassList[$iALPmass]}_RunPlotter_job_${YearsList[$iYear]}_${ChannelList[$iChannel]}.err -argu ${ALPmassList[$iALPmass]} ${YearsList[$iYear]} ${ChannelList[$iChannel]}
      done
    done
  done
