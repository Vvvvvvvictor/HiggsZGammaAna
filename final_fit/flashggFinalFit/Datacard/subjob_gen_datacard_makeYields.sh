...
for ((iChannel=0; iChannel<$nChannel; iChannel++))
  ...
  for ((iALPmass=0; iALPmass<$nALPmass; iALPmass++))
    ...
    for ((iYear=0; iYear<$nYear; iYear++))
      do
        hep_sub $path_makeYields/runjob_gen_datacard_makeYields.sh -g cms -mem 2000 -wt mid -o $path_datacard_log/${ChannelList[$iChannel]}/${ALPmassList[$iALPmass]}_makeYields_job_${YearsList[$iYear]}_${ChannelList[$iChannel]}.log -e $path_datacard_log/${ChannelList[$iChannel]}/${ALPmassList[$iALPmass]}_makeYields_job_${YearsList[$iYear]}_${ChannelList[$iChannel]}.err -argu ${YearsList[$iYear]}=$path_inputWSDir ${ALPmassList[$iALPmass]} ${YearsList[$iYear]} ${ChannelList[$iChannel]}
      done
    done
  done