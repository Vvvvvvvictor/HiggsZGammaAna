...
void DrawMyPlots(string Object1, string Object2, string Selections,  string XTitle, string YUnit, string PlotName, int BinTotal, double BinXLow, double BinXHig, double mySys, int LegendLR, int IfLogY, int IfEB=-1){
  ...
  //====data=======
  TString variable_Data = Object1 + ">>Histo_Data_temp1" + Taolimits;
  Data_Tree->Draw(variable_Data, Selections.c_str());
  TH1D *h_data = (TH1D*)gDirectory->Get("Histo_Data_temp1");
  h_data->SetTitle("");
  c1->Clear();

  TString variable_Data2 = Object2 + ">>Histo_Data_temp2" + Taolimits;
  Data_Tree->Draw(variable_Data2, Selections.c_str());
  TH1D *h_data2 = (TH1D*)gDirectory->Get("Histo_Data_temp2");
  h_data2->SetTitle("");
  c1->Clear();
  h_data->Sumw2();
  h_data2->Sumw2();
  ...
  TString variable_MC = Object1 + ">>Histo_MC_temp" + Taolimits;
  MC_Tree->Draw(variable_MC, Selections.c_str());
  TH1D *h_MC = (TH1D*)gDirectory->Get("Histo_MC_temp");
  c1->Clear();
  TString variable_MC2 = Object2 + ">>Histo_MC_temp2" + Taolimits;
  MC_Tree->Draw(variable_MC2, Selections.c_str());
  TH1D *h_MC2 = (TH1D*)gDirectory->Get("Histo_MC_temp2");
  c1->Clear();
  h_MC->Sumw2();
  h_MC2->Sumw2();
  h_MC->Add(h_MC2);
  ...
  TString ObjectPlusSys = Form("%f*", 1.0+mySys) + Object1;
  ...
  TString ObjectPlusSys2 = Form("%f*", 1.0+mySys) + Object2;
  //TString ObjectPlusSys = "pho_MVAOutputCorr_Down1Sigma";

  TString variable_MCP2 = ObjectPlusSys2 + ">>Histo_MCP_temp2" + Taolimits;

  MC_Tree->Draw(variable_MCP2, Selections.c_str());
  TH1D *h_MCP2 = (TH1D*)gDirectory->Get("Histo_MCP_temp2");
  c1->Clear();
  h_MCP->Sumw2();
  h_MCP2->Sumw2();
  h_MCP->Add(h_MCP2);
  ...
  TString ObjectMinusSys = Form("%f*", 1.0-mySys) + Object1;
  ...
  c1->Clear();
  TString ObjectMinusSys2 = Form("%f*", 1.0-mySys) + Object2;
  //TString ObjectMinusSys = "pho_MVAOutputCorr_Up1Sigma";
  TString variable_MCM2 = ObjectMinusSys2 + ">>Histo_MCM_temp2" + Taolimits;
  MC_Tree->Draw(variable_MCM2, Selections.c_str());
  TH1D *h_MCM2 = (TH1D*)gDirectory->Get("Histo_MCM_temp2");
  c1->Clear();
  h_MCM->Sumw2();
  h_MCM2->Sumw2();
  h_MCM->Add(h_MCM2);
...
//
  ...
  */
  ...
  DrawMyPlots("pho1R9","pho2R9",selection, "phoR9", "", "phoR9_UL", 50, 0., 1.1, 0.005, 1, 0, 0);
  DrawMyPlots("pho1IetaIeta55","pho2IetaIeta55",selection, "phoIetaIeta55", "", "phoIetaIeta55_UL", 50, 0., 0.07, 0.01, 0, 0, 0);
  DrawMyPlots("pho1PIso_noCorr","pho2PIso_noCorr",selection, "phoPIso_noCorr", "", "phoPIso_noCorr_UL", 50, 0., 40.0, 0.01, 0, 1, 0);
  ...
  //DrawMyPlots("ALP_calculatedPhotonIso",selection, "ALP_calculatedPhotonIso", "", "ALP_calculatedPhotonIso_UL", 50, 0., 5, 0.01, 0, 1, 0);
