...
void draw_HMassDis_sum(TString mode = "zero_jet", int button = 0)
{
    ...
    can->SaveAs(Form("%s/plots/h_mass_dis/%s_sum.pdf", str_dir.Data(), mode.Data()));

    cout << Form("\tPath is %s", str_dir.Data()) << endl;
    cout << Form("\tWe get the PICTURE:%s!!!!!!!", Form("%s/plots/h_mass_dis/%s_sum.pdf", str_dir.Data(), mode.Data())) << endl;
}
