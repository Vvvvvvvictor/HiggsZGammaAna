...
void draw_com_mc_bdt(int type = 2)
{
    ...
    TString cate_name[5] = {"zero_jet", "one_jet", "two_jet", "VH_ttH", "0"}, channel, mode = "test" ;
    if (type == 4) channel = "Inclusive";
    else channel = cate_name[type];

    //TODO: check things below!
    TString x_name = "bdt_score_t";
    Float_t data;
    double x_axis[3] = {100, 0, 1};
    TString XTitle = Form("%s:bdt_score_t", channel.Data());
    TString YTitle = Form("Events/%.2g" , (x_axis[2]-x_axis[1])/x_axis[0]);

    TString str_dir = "/afs/cern.ch/user/j/jiehan/private/hmumuml";
    TString stype[][5] = {{"DYJetsToLL", "0"}, {"ZGToLLG", "LLAJJ", "TTG", "TG", "0"}};//, {"dy", "0"}, {"tt", "0"}, {"llajj", "0"}, {"ww", "wz", "zz", "0"}};
    ...
    can->SaveAs(Form("%s/plots/bdt_dis/mc/mc_%s_%s.pdf", str_dir.Data(), x_name.Data(), channel.Data()));
