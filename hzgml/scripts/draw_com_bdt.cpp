...
void draw_com_bdt(int type = 4)
{
    ...
    TString cate_name[] = {"zero_jet", "one_jet", "two_jet", "VH_ttH", "0"}, channel, mode = "test";
    if (type == 4) channel = "Inclusive";
    else channel = cate_name[type].Data();

    TString s_LG[] = {"Fake photons", "True photons"};//"SM ZG", "VBS ZG", "TTG", "TG"};
    TString l_LG[] = {"data"};
    ...
    TString XTitle = Form("%s: Score", channel.Data());
    TString YTitle = Form("Events/%.2g" , (x_axis[2]-x_axis[1])/x_axis[0]);
    
    TString str_dir = "/afs/cern.ch/user/j/jiehan/private/hmumuml";
    TString stype[][5] = {{"data_fake", "mc_true", "0"}, {"mc_med", "0"}};//{"zg", "llajj", "ttg", "tg", "0"}};//, {"llajj", "0"}, {"ttg", "0"}, {"tg", "0"}, {"dy", "0"}, {"tt", "0"}, {"llajj", "0"}, {"ww", "wz", "zz", "0"}};
    TString ltype[][4] = {{"data_med", "0"}};
    ...
    Double_t w, gam_w, ele_w, mu_w, fw, tw, cut1, cut2, cut3, cut4, cut5, cut6, cut7, cut8, cut9, n;
    Float_t bdt;
    ...
    for (int typei = 0; typei < stypes; typei++)
    {
        ...
        while(stype[typei][len] != "0"){
            int catei = -1;
            while (cate_name[catei+1] != "0"){
            catei++;
            if (type<4) if (catei!=type) continue;
            cout << "\n\tLoading file" << Form("%s/outputs/%s/%s.root", str_dir.Data(), cate_name[catei].Data(), stype[typei][len].Data()) << "!......" << endl;

            TFile *f = new TFile(Form("%s/outputs/%s/%s.root", str_dir.Data(), cate_name[catei].Data(), stype[typei][len].Data()));
            ...
            // t->SetBranchAddress("Z_sublead_lepton_pt", &cut2);
            // t->SetBranchAddress("gamma_relpt", &cut3);
            // t->SetBranchAddress("gamma_pt", &cut4);
            // t->SetBranchAddress("gamma_energyErr", &cut5);
            ...
            for (Long64_t i = 0; i < nentries; i++)
            {
                ...
                // if (cut2<15) continue;
                // if (bdt<0 | bdt>1) continue;
                // if (typei==1) w = w * tw; //TODO: check it
                // if (typei==0) w = w * fw;
    ...
    for (int typei = 0; typei < ltypes; typei++)
    {
        ...
        while (ltype[typei][len] != "0")
        {
            ...
            catei++;
            if (type<4) if (catei!=type) continue;
            cout << "\n\tLoading file" << Form("%s/outputs/%s/%s.root", str_dir.Data(), cate_name[catei].Data(), ltype[typei][len].Data()) << "!......" << endl;

            TFile *f = new TFile(Form("%s/outputs/%s/%s.root", str_dir.Data(), cate_name[catei].Data(), ltype[typei][len].Data()));
            ...
            t->SetBranchAddress("H_mass", &cut1);
            // t->SetBranchAddress("Z_sublead_lepton_pt", &cut2);
            // t->SetBranchAddress("gamma_mvaID", &cut3);
            // t->SetBranchAddress("gamma_pt", &cut4);
    ...
    data_h->GetYaxis()->SetTitle("data/bkg");
    ...
    // can->SetGrid(2, 1);
    can->SaveAs(Form("%s/plots/bdt_dis/fwtw/dd_%s_%s.pdf", str_dir.Data(), x_name.Data(), channel.Data()));//TODO: check it
