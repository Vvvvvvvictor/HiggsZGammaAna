...
void draw_com(TString mode = "inclusive")
{
    ...
    TString s_LG[] = {"Fake photons","SM ZG", "VBS ZG", "DY(not overlap)", "TTG", "TG"};
    TString l_LG[] = {"data"};
    double Y_axis[3] = {0, 0.02, 100};
    double pos[2] = {0.75, 0.95};
    double ratio = 1.1; // the ratio of top limit to the highest bin
    int stypes = 6, ltypes = 1;

    //TODO: check things below!
    TString x_name = "gamma_pt";
    Double_t data;
    double x_axis[3] = {120, 15, 75};
    TString XTitle = "P_{T}^{#gamma}[GeV/c]";
    TString YTitle = Form("Events/%.2gGeV/c" , (x_axis[2]-x_axis[1])/x_axis[0]);
    
    TString str_dir = "/afs/cern.ch/user/j/jiehan/private/hmumuml";
    TString stype[][5] = {{"data_fake", "mc_true", "0"}, {"mc_med", "0"}, {"mc_med", "0"}, {"mc_med", "0"}, {"mc_med", "0"}, {"mc_med", "0"}};//{"zg", "llajj", "ttg", "tg", "0"}};//, {"llajj", "0"}, {"ttg", "0"}, {"tg", "0"}, {"dy", "0"}, {"tt", "0"}, {"llajj", "0"}, {"ww", "wz", "zz", "0"}};
    ...
    Double_t w, fw, tw, cut1, cut2, cut3, cut4, cut5, cut6, cut7, cut8, cut9;
    Long64_t n, tag;
    ...
    for (int typei = 0; typei < stypes; typei++)
    {
        ...
        while(stype[typei][len] != "0"){
            ...
            for (Long64_t i = 0; i < nentries; i++)
            {
                ...
                // if (cut2<15) continue;
                // if (n!=0) continue;
                // if (typei>0) w = w * tw; //TODO: check it
                // if (typei==0) w = w * fw;
                if (abs(cut4) < 1.5) continue;
                if ( (cut1 > 120 & cut1 < 130))  w = 0;
                if (typei) if (typei!=tag) continue;
    ...
    // can->SetGrid(2, 1);
    can->SaveAs(Form("%s/overlap/figures/com/fwtw/dd_%s_%s.pdf", str_dir.Data(), x_name.Data(), mode.Data()));//TODO: check it
