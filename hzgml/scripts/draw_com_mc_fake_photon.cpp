...
void draw_com_mc_fake_photon(TString mode = "inclusive", int lep_id = 13, int isbarrel = 1)
{
    ...
    TString s_LG[] = {"SB: dy ", "CR: dy "};//"SM ZG", "VBS ZG", "TTG", "TG"};
    TString l_LG[] = {"SR: dy, "};
    double Y_axis[3] = {0, 0.02, 100};
    double pos[2] = {0.6, 0.95};
    double ratio = 1.3; // the ratio of top limit to the highest bin
    int stypes = 2, ltypes = 1;

    int ptbins_temp1[] = {15,17,20,25,30,40,50,5000};
    ...
    TString x_name = "Z_pt";
    Double_t data;
    double x_axis[3] = {24, 0, 120};
    TString str_dir = "/afs/cern.ch/user/j/jiehan/private/hmumuml";
    TString ftem[][5] = {{"dy", "0"}, {"dy", "0"}};//{"zg", "llajj", "ttg", "tg", "0"}};//, {"llajj", "0"}, {"ttg", "0"}, {"tg", "0"}, {"dy", "0"}, {"tt", "0"}, {"llajj", "0"}, {"ww", "wz", "zz", "0"}};
    TString fref[][4] = {{"dy", "0"}};
    ...
    cout << "\n\tTarget: " << Form("%s/overlap/figures/com/check_fake_template_dy_%s_%s_%s_%s.pdf", str_dir.Data(), x_name.Data(), mode.Data(), region.Data(), lepton.Data()) << endl;
    // cout << "\n\tTarget: " << Form("%s/overlap/figures/com/check_fake_template_dy_%s_%s_%s_%s_pt%d_pt%d.pdf", str_dir.Data(), x_name.Data(), mode.Data(), region.Data(), lepton.Data(), ptbins[bini], ptbins[bini+1]) << endl;
    ...
    Double_t w, fw, tw, iso, cut1, cut2, cut3, cut4, cut5, cut6, cut7, cut8, cut9;
    Long64_t n, id;
    ...
    for (int typei = 0; typei < stypes; typei++)
    {
        ...
        while(ftem[typei][len] != "0"){
            cout << "\n\tLoading file" << Form("%s/overlap/%s.root", str_dir.Data(), ftem[typei][len].Data()) << "!......" << endl;

            TFile *f = new TFile(Form("%s/overlap/%s.root", str_dir.Data(), ftem[typei][len].Data()));
            ...
            for (Long64_t i = 0; i < nentries; i++)
            {
                ...
                // if (iso*cut3>5) continue;
                // if (cut6<-0.97 & iso*cut3>1.5) continue;
                if (isbarrel==1){
                    if (abs(cut5)>1.5) continue;
                    if ((typei==0) & (cut6<-0.97 | cut6>-0.4)) continue;
                    if ((typei==1) & (cut6<-0.4 | cut6>0.42)) continue;
                    if (iso*cut3>1.141) continue;
                }
                if (isbarrel==0){
                    if (abs(cut5)<1.5) continue;
                    if ((typei==0) & (cut6<-0.97 | cut6>-0.59)) continue;
                    if ((typei==1) & (cut6<-0.59 | cut6>0.14)) continue;
    ...
    for (int typei = 0; typei < ltypes; typei++)
    {
        ...
        {
            cout << "\n\tLoading file" << Form("%s/overlap/%s.root", str_dir.Data(), fref[typei][len].Data()) << "!......" << endl;

            TFile *f = new TFile(Form("%s/overlap/%s.root", str_dir.Data(), fref[typei][len].Data()));
    ...
    fref_h->GetYaxis()->SetTitle(Form("Rel. value/%.2gGeV/c" , (x_axis[2]-x_axis[1])/x_axis[0]));
    fref_h->GetYaxis()->SetTitleOffset(1.6);
    fref_h->SetMaximum(max * ratio);
    can->Update();
    cout << "\n\tFinish drawing lines" << endl;

    TLegend *legend = new TLegend(pos[0], 0.8, pos[1], 0.95);
    for (int typei = 0; typei < stypes; typei++)
    ...
    // if (bini==num_ptbin-1) text->DrawLatex(0.375, 0.875, Form("%s, %s, P_{T}^{#gamma} > %d", region.Data(), lepton.Data(), ptbins[bini])); // TODO: check it before run
    // // else if (i==0) text->DrawLatex(0.5, 0.5, Form("%s < %1.1f", y_lg.Data(), max)); // TODO: check it before run
    // else // TODO: check it before run
    //     text->DrawLatex(0.375, 0.875, Form("%s, %s, %d < P_{T}^{#gamma} < %d", region.Data(), lepton.Data(), ptbins[bini], ptbins[bini+1]));
    text->DrawLatex(0.375, 0.875, Form("%s, %s", region.Data(), lepton.Data()));
    ...
    div_h->GetYaxis()->SetTitle("SR/SB");
    div_h->GetYaxis()->SetTitleOffset(1.6); 
    div_h->GetXaxis()->SetTickLength(0.1);
    div_h->GetXaxis()->SetTitle(Form("%s[GeV/c]", x_name.Data()));
    can->Update();
    cout << "\tFinish drawing subpad!!!" << endl;

    // can->SetGrid(2, 1);
    // can->SaveAs(Form("%s/overlap/figures/com/dy_template/check_fake_template_dy_%s_%s_%s_%s_pt%d_pt%d.pdf", str_dir.Data(), x_name.Data(), mode.Data(), region.Data(), lepton.Data(), ptbins[bini], ptbins[bini+1]));
    can->SaveAs(Form("%s/overlap/figures/com/dy_template/check_fake_template_dy_%s_%s_%s_%s.pdf", str_dir.Data(), x_name.Data(), mode.Data(), region.Data(), lepton.Data()));

    // cout << Form("\tPath is %s", str_dir.Data()) << endl;
    // cout << Form("\tWe get the PICTURE:%s!!!!!!!", Form("%s/overlap/figures/com/check_fake_template_dy_%s_%s_%s_%s_pt%d_pt%d.pdf", str_dir.Data(), x_name.Data(), mode.Data(), region.Data(), lepton.Data(), ptbins[bini], ptbins[bini+1])) << endl;

    // fref_h->Reset("ICES"); //TODO: check it
    // for (int typei = 0; typei < stypes; typei++)
    // {
    //     h1[typei]->Reset("ICES");
    // }
