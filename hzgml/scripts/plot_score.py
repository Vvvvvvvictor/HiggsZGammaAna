...
def getArgs():
    ...
    parser.add_argument('-r', '--region', action = 'store', choices = ['all_jet', 'two_jet', 'one_jet', 'zero_jet', 'VBF', 'VH_ttH'], default = 'zero_jet', help = 'Region to process')
...
def getYields(args, region, sample, sampleConfig, edges, variable):
    ...
    for d in tqdm(read_root(file_list, key='test', columns=[variable, weight, 'H_mass'], chunksize=500000), desc=f'Loading {sample}', bar_format='{desc}: {percentage:3.0f}%|{bar:20}{r_bar}'):
...
def main():
    ...
    region_tag = {"zero_jet": "0-jet", "one_jet": "1-jet", "two_jet": "2-jet", "VBF": "2-jet", "VH_ttH": "3-lepton"}

    samples = OrderedDict([
            ('Signal', {'process': ['sig'], 'selection': ["H_mass >= 120", "H_mass <= 130"], 'eventWeight': 'weight', 'normFactor': 100, 'color': "orange", 'ls': '-.', 'plotType': 'signal'}),
            ('Data sideband', {'process': ['data'], 'selection': ["(H_mass <= 120) | (H_mass >= 130)"], 'eventWeight': 'weight', 'normFactor': 0.20, 'color': "on:pink", 'plotType': 'bkg'})
        ])

    edges = np.linspace(0, 1, 21)

    variable = f"{args.variable}_score{'_VBF' if args.region == 'VBF' else ''}{'_t' if args.transform else ''}"
    variable_name = f"${args.variable.upper()}_{{VBF}}$" if args.region == "VBF" else f"${args.variable.upper()}$"    

    plot_score(args, f"{args.variable.upper()}_{args.region}", region, samples, edges, f"{region_tag[region]}", variable, variable_name, doLogY=True)
