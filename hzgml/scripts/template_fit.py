...
def AddHist_data(file, hist, x, ptrange, isbarrel, lep_sel):
    branches = uproot.open(file+':inclusive').arrays(['H_mass','gamma_pt','gamma_eta', 'gamma_chiso', "gamma_mvaID",'weight','Z_lead_lepton_id', 'Z_sublead_lepton_pt', 'Z_pt'], library='pd')

    arrays = branches.loc[(branches.loc[:, 'Z_sublead_lepton_pt'] > 15) & ((branches.loc[:, 'H_mass'] < 120) | (branches.loc[:, 'H_mass'] > 130)), :].copy()
        
    if isbarrel == 1:
        eta_cut = (arrays.loc[:,'gamma_eta'] >-1.5) & (arrays.loc[:,'gamma_eta'] <1.5)
        mvaid_cut = arrays.loc[:,'gamma_mvaID'] > 0.42
    else:
        eta_cut = (arrays.loc[:,'gamma_eta'] <-1.5) | (arrays.loc[:,'gamma_eta'] >1.5)
        mvaid_cut = arrays.loc[:,'gamma_mvaID'] > 0.14
    lepton_cut =  (arrays.loc[:, 'Z_lead_lepton_id'] == lep_sel)
    range_cut = (arrays.loc[:, 'Z_pt'] > x[0]) & (arrays.loc[:, 'Z_pt'] < x[1])
    pt_cut = (arrays.loc[:,'gamma_pt'] >= ptrange[0]) & (arrays.loc[:,'gamma_pt'] < ptrange[1])

    arrays = arrays.loc[range_cut & pt_cut & eta_cut  & lepton_cut & mvaid_cut,:]
    
    for i in trange(0, len(arrays['Z_pt']), desc=f'fill pt of Z for {file}'):
        hist.Fill(float(arrays['Z_pt'].values[i]))

    return hist
    
    
def AddHist_mcTruth(file, hist, x, ptrange, isbarrel, lep_sel):
    branches = uproot.open(file+':inclusive').arrays(['H_mass','gamma_pt','gamma_eta', 'gamma_chiso','gamma_genPartFlav', "gamma_mvaID",'weight','Z_lead_lepton_id', 'Z_sublead_lepton_pt', 'Z_pt', 'n_iso_photons'], library='pd')

    arrays = branches.loc[(branches.loc[:, 'Z_sublead_lepton_pt'] > 15) & ((branches.loc[:, 'H_mass'] < 120) | (branches.loc[:, 'H_mass'] > 130)), :].copy()

    if 'zg' in file:
        pt_cut = ((arrays.loc[:,'gamma_pt'] < 20) & (arrays.loc[:,'n_iso_photons'] < 1)) | (arrays.loc[:,'gamma_pt'] > 20)
    elif 'dy' in file:
        pt_cut = (arrays.loc[:,'gamma_pt'] < 20) & (arrays.loc[:,'n_iso_photons'] > 0)
    else:
        pt_cut = arrays.loc[:,'gamma_pt'] > 0
    # pt_cut = arrays.loc[:,'gamma_pt'] > 0
    ...
    lepton_cut =  (arrays.loc[:, 'Z_lead_lepton_id'] == lep_sel)
    range_cut = (arrays.loc[:, 'Z_pt'] > x[0]) & (arrays.loc[:, 'Z_pt'] < x[1]) 
    pt_cut = pt_cut & (arrays.loc[:,'gamma_pt'] >= ptrange[0]) & (arrays.loc[:,'gamma_pt'] < ptrange[1]) 
    gen_cut = arrays.loc[:,'gamma_genPartFlav'] == 1

    arrays = arrays.loc[range_cut & pt_cut & eta_cut & gen_cut & lepton_cut & mvaid_cut,:]
    
    for i in trange(0, len(arrays['Z_pt']), desc=f'fill pt of Z for {file}'):
        hist.Fill(float(arrays['Z_pt'].values[i]), float(arrays['weight'].values[i]))

    return hist
        
    
def AddHist_dataFake(file, hist, x, ptrange, isbarrel, lep_sel, minus=0):
    if minus == 0:
        branches = uproot.open(file+':inclusive;1').arrays(['H_mass','gamma_pt','gamma_eta', 'gamma_sieie', 'gamma_hoe', 'gamma_chiso',"gamma_mvaID",'weight','Z_lead_lepton_id', 'Z_sublead_lepton_pt', 'Z_pt'], library='pd')
    else:
        branches = uproot.open(file+':inclusive;1').arrays(['H_mass','gamma_pt','gamma_eta', 'gamma_sieie', 'gamma_hoe', 'gamma_chiso','gamma_genPartFlav', "gamma_mvaID", 'weight','Z_lead_lepton_id', 'Z_sublead_lepton_pt', 'Z_pt', 'n_iso_photons'], library='pd')

    arrays = branches.loc[(branches.loc[:, 'Z_sublead_lepton_pt'] > 15) & ((branches.loc[:, 'H_mass'] < 120) | (branches.loc[:, 'H_mass'] > 130)), :].copy()

    if 'DY' in file:
        pt_cut = (arrays.loc[:,'n_iso_photons'] < 0)
    else:
        pt_cut = arrays.loc[:,'gamma_pt'] > 0
        
    if isbarrel == 1:
        eta_cut = (arrays.loc[:,'gamma_eta'] >-1.5) & (arrays.loc[:,'gamma_eta'] <1.5)
        mvaid_cut = (arrays.loc[:,'gamma_mvaID'] < -0.4) & (arrays.loc[:,'gamma_mvaID'] > -0.97)
        chiso_cut = (arrays['gamma_chiso']*arrays['gamma_pt'] < 1.141)
    else:
        eta_cut = (arrays.loc[:,'gamma_eta'] <-1.5) | (arrays.loc[:,'gamma_eta'] >1.5)
        mvaid_cut = (arrays.loc[:,'gamma_mvaID'] < -0.59) & (arrays.loc[:,'gamma_mvaID'] > -0.97)
        chiso_cut = (arrays['gamma_chiso']*arrays['gamma_pt'] < 1.051)
    lepton_cut =  (arrays.loc[:, 'Z_lead_lepton_id'] == lep_sel)
    range_cut = (arrays.loc[:, 'Z_pt'] > x[0]) & (arrays.loc[:, 'Z_pt'] < x[1])     
    pt_cut = pt_cut & (arrays.loc[:,'gamma_pt'] >= ptrange[0]) & (arrays.loc[:,'gamma_pt'] < ptrange[1])
    if minus == 0:
        arrays = arrays.loc[range_cut & pt_cut & eta_cut & lepton_cut & mvaid_cut & chiso_cut,:]
    else:
        gen_cut = arrays.loc[:,'gamma_genPartFlav'] == 1
        arrays = arrays.loc[range_cut & pt_cut & eta_cut & lepton_cut & gen_cut & mvaid_cut & chiso_cut,:]
    ...
    if minus == 0:
        ...
        for i in trange(0, len(arrays['Z_pt']), desc=f'fill pt of Z for {file}'):
            hist.Fill(float(arrays['Z_pt'].values[i]), -1*float(arrays['weight'].values[i]))
...
if __name__ == '__main__':
    ...
    for i, ptlow in enumerate(ptlist[:len(ptlist)-1]):
        ...
        ntrue = ROOT.RooRealVar("true number", "true number", 0.7*ndata, 1, ndata)
        nfake = ROOT.RooRealVar("fake number", "fake number", 0.3*ndata, 1, ndata)
        ...
        xframe.GetYaxis().SetRangeUser(0, hist_data.GetBinContent(hist_data.GetMaximumBin()) * 1.6)
        ...
        xframe.GetXaxis().SetTitle("P_{T}^{ll}")
        ...
        legend = ROOT.TLegend(legend_left-0.35, 0.675, legend_right-0.35, 0.875)
        ...
        textChi2.DrawLatex(legend_left, 0.825, "#chi^{2}/n="+str("%.2f" % chi2ToNDF))
        textChi2.DrawLatex(legend_left, 0.775, str(ptrange[0])+" GeV < P_{T}^{#gamma} < "+str(ptrange[1])+" GeV")
        ...
        textChi2.DrawLatex(legend_left, 0.725, "Fake Fraction: "+ str("%.3f" % fake_fraction) + "#pm " + str("%.3f" % fake_fraction_err)) 

        # true_fraction = ntrue_window / (nfake_window + ntrue_window)
        # true_fraction_err = numpy.sqrt(pow(ntrue_window/pow(ntrue_window+nfake_window,2),2)*pow(nfake_window_err,2) + pow(nfake_window/pow(nfake_window+ntrue_window,2),2)*pow(ntrue_window_err,2))
        # textChi2.DrawLatex(legend_left, 0.70, "True Fraction: "+ str("%.3f" % true_fraction) + "#pm " + str("%.3f" % true_fraction_err)) 
