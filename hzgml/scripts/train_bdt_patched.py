...
def getArgs():
    ...
    parser.add_argument('--planing-bins', type=int, default=0, help='If >0, flatten background mass in training with N bins')
...
========================================================================
    ...
    def prepareData(self, fold=0):
        ...
        # Optional mass planing to reduce mass sculpting on background (training weights only)
        if hasattr(self, '_args') and getattr(self._args, 'planing_bins', 0) and self._args.planing_bins > 0:
            nb = int(self._args.planing_bins)
            if 'mass' in train_bkg.columns:
                bkg_m = train_bkg['mass'].to_numpy()
                m_min = float(np.nanmin(bkg_m))
                m_max = float(np.nanmax(bkg_m))
                bins = np.linspace(m_min, m_max, nb+1, dtype=float)
                counts, _ = np.histogram(bkg_m, bins=bins)
                counts = np.where(counts==0, 1, counts)
                idx = np.clip(np.digitize(bkg_m, bins)-1, 0, nb-1)
                scale = 1.0 / counts[idx]
                scale *= counts.sum() / scale.sum()
                # train_bkg_wt is a Series; align by index before scaling
                scaled_bkg = train_bkg_wt * pd.Series(scale, index=train_bkg_wt.index)
                self.m_train_wt[fold] = pd.concat([train_sig_wt, scaled_bkg]).to_numpy()
                print(f'[Planing] Applied background mass flattening with {nb} bins in [{m_min:.1f}, {m_max:.1f}].')
            else:
                print('[Planing] WARNING: no "mass" column; skip planing.')
