...
correctionlib.schemav2.IGNORE_FLOAT_INF = True
...
class Report:
    ...
    def generate(self, files):
        ...
        if (
            len(self.corr_version_error)
            + len(self.corr_removed)
            + len(self.corr_added)
            + len(self.corr_updated)
            == 0
        ):
...
def compare_files(old, new):
    ...
    for corr in corrs_new:
        ...
        old_same_version = next(
            (c for c in corrs_old_same_name if c["version"] == vers), None
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compares the content of two correction JSON files. Assumes their content are valid schemav2."
    )
