...
class ZGammaTaggerOverlap(Tagger):
    def __init__(self, name = "default_zgamma_tagger", options = {}, is_data = None, year = None):
        super(ZGammaTaggerOverlap, self).__init__(name, options, is_data, year)
    ...
    def calculate_selection(self, events):
        """
        Select photons and create zgamma pairs.
        Add a record "zgammas" to events array with relevant information about each zgamma pair.
    ...
    def produce_and_select_zgammas(self, events, photons, options):
        ...
        For events with more than 2 leptons and 1 gamma, more than 1 zgamma candidate
        per event is possible.

        :param events: events array to calculate zgamma candidates from
        ...
        # self.register_cuts(
        #     names = ["trigger_cut", "has_z_cand", "has_gamma_cand", "all cuts"],
        #     results = [trigger_cut, has_z_cand, has_gamma_cand, all_cuts],
        #     cut_type = "zgammas"
        # )
    ...
    def select_photons(self, photons, rho, options):
        ...
        # Photon ID cut
        pho_id = photons.mvaID_WP90 == options["mvaID_WP90"]
        ...
        all_cuts = pt_cut & eta_cut & e_veto_cut & pho_id

        # self.register_cuts(
        #         names = ["pt > %.4f" %(options['pt']), "eta", "e_veto", "all"],
        #         results = [pt_cut, eta_cut, e_veto_cut, all_cuts],
        #         cut_type = "photon"
        # )
        self.register_cuts(
                names = ["pt > %.4f" %(options['pt']), "eta", "e_veto", "photon_id", "all"],
                results = [pt_cut, eta_cut, e_veto_cut, pho_id, all_cuts],
                cut_type = "photon"
        )
