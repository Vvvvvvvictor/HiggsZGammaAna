...
RooAbsPdf* getBernsteinxZGMCShape(string prefix, int order, RooRealVar* obs_var){
  ...
  for (int i=0; i<order; i++){
    ...
    RooRealVar *param = new RooRealVar(name.c_str(),name.c_str(),0.3*(8-i),-10.,10.);
...
void SSTest_core_function(int cat = 0, int sig = 0, TString channel = "two_jet"){
  //background MC template
  TH1F* hbkg;
  TFile* fbkg = TFile::Open(Form("./bkg_sig_template.root"));
  ...
  //sigPdf MC
  TFile* fsig = TFile::Open(Form("./bkg_sig_template.root"));
  ...
  //data side band
  TFile* fsb = TFile::Open(Form("./bkg_sig_template.root"));
  ...
  for (vector<string>::iterator funcType=functionClasses.begin(); funcType!=functionClasses.end(); funcType++){
    ...
    while (order < 2){ //FIXME
      ...
      if (bkgPdf){
        ...
        CMS_hzg_mass->setRange("R1",mgg_low,122);
        CMS_hzg_mass->setRange("R2",122,128);
        CMS_hzg_mass->setRange("R3",128,mgg_high);

        //background function fit
        bkgPdf_fit = bkgPdf->fitTo(*dsb,CutRange("R1,R3"),RooFit::Save(1),RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE)); 
        // nllt = RooNLLVar("nllt","nllt",bkgPdf,*dsb);
        ...
        output << "\t" << bkg_fun.Data() << "\tsb:\tnpars = " << bkg_npars << " \tchi^2 = " << chi2 << "\tprob = " << prob << endl;
        ...
        frame_bkg->Draw();
        gPad->Print(Form("./test1/data_sb_shape_%s_cat%d_%s.pdf",channel.Data(),cat,bkg_fun.Data()));

        cout << "\t=================================" << endl;
        cout << "\n\t Finish data side band fit\n" << endl;

        // MC background fitting
        bkgMCPdf_fit = bkgPdf->fitTo(*dbkg,RooFit::Save(1),RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE), RooFit::Extended(kTRUE));

        bkg_npars = bkgMCPdf_fit->floatParsFinal().getSize();
        ...
        // if (prob < 0.05) status = "Fail";
        // bkgPdf->Write(bkgPdf->GetName(), TObject::kOverwrite);
        output << "\t" << bkg_fun.Data() << "\tbkg:\tnpars = " << bkg_npars << " \tchi^2 = " << chi2 << "\tprob = " << prob << endl;
        frame_bkg->Draw();
        // RooHist *hpull = frame_bkg->pullHist();
        // RooPlot *frame3 = CMS_hzg_mass->frame(Title("Pull Distribution"));
        // frame3->addPlotable(hpull, "P");
        // frame3->Draw();
        gPad->Print(Form("./test1/mc_bkg_shape_%s_cat%d_%s.pdf",channel.Data(),cat,bkg_fun.Data()));
        ...
        frame_sig->Draw();
        gPad->Print(Form("./test1/sigPdf_shape_%s_cat%d_%s.pdf",channel.Data(),cat,bkg_fun.Data()));
        ...
        model_fit = model->fitTo(*ddata,RooFit::Save(1),RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE), RooFit::Extended(kTRUE)); //FIXME kTRUE or kFALSE
        dmc = nsig.getError();
        ...
        //gPad->Print(Form("./test1/nllmc_shape_%s_cat%d_%s.pdf",channel.Data(),cat,bkg_fun.Data()));
        ...
        //RooPlot* frame_ss = nsig.frame(RooFit::Bins(20),RooFit::Range(ss-2*dss,ss+2*dss),RooFit::Title("LL in ss")) ;
        //nll_ss->plotOn(frame_ss,ShiftToZero());
        //frame_ss->Draw();
        //gPad->Print(Form("./test1/nllss_shape_%s_cat%d_%s.pdf",channel.Data(),cat,bkg_fun.Data()));
        ...
        canv->SaveAs(Form("./test1/pesudo_data_shape_%s_cat%d_%s.pdf",channel.Data(),cat,bkg_fun.Data()));
