...
for primary_process_name, tree_name in Tree_dict.items(): 
    ...
    TREE = tree_name
    WEIGHT = "weight"
    PATH = "/eos/home-p/pelai/HZgamma/Root_Dataset/run3_jet_horn_pt50/NanoV12/"
    ...
    for i in config_dict:
        ...
        if os.path.exists(f"pic/run3_jet_horn_pt50/dataVbkg_{primary_process_name}") == False:
            os.makedirs(f"pic/run3_jet_horn_pt50/dataVbkg_{primary_process_name}")
        if primary_process_name == "ggF":
            plt.savefig(f"pic/run3_jet_horn_pt50/dataVbkg_{primary_process_name}/{name_dict_ggF[VAR]}.pdf")
        elif primary_process_name == "VBF":
            plt.savefig(f"pic/run3_jet_horn_pt50/dataVbkg_{primary_process_name}/{name_dict_VBF[VAR]}.pdf")
