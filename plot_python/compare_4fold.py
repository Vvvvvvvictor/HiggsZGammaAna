...
ratio = 500
bkg_legend = ["SM ZG", "DYJets", "LWK ZG", "TT", "Diboson"]
sig_legend = ["sig"]
...
for i in range(4):
    ...
    fold_selection = ["event%4 == {}".format(i)]
    ...
    #             arrays = pic.read_file(path+channel+"/"+sub_bkg, var, tree, hist_selections)
    #             if k != 0:
    ...
    #     else: 
    #         arrays = pic.read_file(path+channel+"/"+bkg, var, tree, hist_selections)
    #         file_hist, _, yields = pic.get_hist(arrays, var, 1, "subbkg_{}".format(j), bins, x_range)
    ...
    for j, sig in enumerate(sig_file_list):
        if isinstance(sig, list):
            for k, sub_sig in enumerate(sig):
                arrays = pic.read_file(path+channel+"/"+sub_sig, var, tree, hist_selections)
                if k != 0:
            ...
            arrays = pic.read_file(path+channel+"/"+sig, var, 1, tree, hist_selections)
...
# for i in range(4):
#     legend.AddEntry("bkg_mc_{}".format(i), "{} fold: event%4=={}".format(fold_legend[i], i), "p")
for i in range(4):
    legend.AddEntry("sig_{}".format(i), "{} fold: event%4=={}".format(fold_legend[i], i), "p")
...
c1.Draw()
c1.SaveAs("/afs/cern.ch/user/j/jiehan/private/HiggsZGammaAna/plot_python/pic/4fold_{}_sig.png".format(channel))
