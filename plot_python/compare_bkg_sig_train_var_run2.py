...
for primary_process_name, tree_name in Tree_dict.items(): 
    ...
    for var in config_dict:
        ...
        plt.tight_layout()
        if os.path.exists(f"pic/run2/sigVbkg_{primary_process_name}") == False:
            os.makedirs(f"pic/run2/sigVbkg_{primary_process_name}")
        
        if primary_process_name == "ggF":
            plt.savefig(f"pic/run2/sigVbkg_{primary_process_name}/{name_dict_ggF[VAR]}.pdf")
        elif primary_process_name == "VBF":
            plt.savefig(f"pic/run2/sigVbkg_{primary_process_name}/{name_dict_VBF[VAR]}.pdf")
