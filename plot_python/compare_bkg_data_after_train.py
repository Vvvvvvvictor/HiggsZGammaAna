...
config_dict = {
    ...
    "bdt_score": {"range": (0, 1), "bins": 50, "title": "BDT score"},
...
TREE = "two_jet"
WEIGHT = "weight"
PATH = f"/eos/user/j/jiehan/root/outputs/test/{TREE}/"
...
def convert_root_to_hist(file_dict, selection=""):
    ...
    for file in file_dict.values():
        ...
        for f in file:
            print("Reading", PATH+f+f".root:{TREE}", "...")
            if "H_mass" not in VAR:
                try:
                    samples = uproot.open(PATH+f+f".root:{TREE}").arrays([VAR, WEIGHT, "H_mass"], library="pd")
                except:
                    continue
            else:
                try:
                    samples = uproot.open(PATH+f+f".root:{TREE}").arrays([VAR, WEIGHT], library="pd")
                except:
                    continue
